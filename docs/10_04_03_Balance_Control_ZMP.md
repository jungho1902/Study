...
- **개념 복습 (파트 10.4.1 연계):** ZMP는 로봇의 발바닥과 지면이 접촉하는 지점에서, 로봇의 움직임으로 인해 발생하는 모든 관성력과 중력이 합쳐져 만들어내는 **전도 모멘트(overturning moment)가 0이 되는 지점**을 의미합니다.
- **물리적 의미:** ZMP는 발바닥에 작용하는 모든 압력의 **분포 중심점**이라고 직관적으로 이해할 수 있습니다. 마치 팽이가 완벽하게 균형을 잡고 돌 때, 그 중심점이 바닥의 한 점에 고정되는 것과 같습니다.
...
- **선형 역진자 모델 (Linear Inverted Pendulum Model, LIPM):**
  - **개념:** 복잡한 휴머노이드 전체의 동역학을 '질량이 한 점에 모여 있는(무게 중심) 막대기가 평면 위에서 움직이는' 매우 단순한 모델로 근사한 것입니다.
  - **가정:** 로봇의 질량이 모두 무게 중심(Center of Mass, CoM)에 모여 있고, 다리의 질량은 무시하며, 로봇의 높이(CoM의 높이)는 일정하게 유지된다고 가정합니다.
...
- **ZMP 피드백 제어:**
  - **문제점:** 이상적인 모델과 달리, 실제 로봇은 모델 오차나 예상치 못한 외력(바닥의 기울어짐, 외부에서 미는 힘 등)으로 인해 계획대로 움직이지 않습니다. 이로 인해 실제 ZMP($`ZMP_{real}`$)는 목표 ZMP($`ZMP_{ref}`$)를 벗어나게 되고, 이는 곧 균형을 잃는 상황으로 이어집니다.
  - **해결책:** 실제 ZMP를 측정하고, 목표 ZMP와의 오차를 보상하도록 실시간으로 제어 입력을 수정합니다.
    1.  **실제 ZMP 측정:** 발바닥에 부착된 **힘/토크 센서(F/T Sensor)**를 통해 지면 반발력과 모멘트를 직접 측정하여 실제 ZMP 위치를 계산합니다.
    2.  **ZMP 오차 계산:** $`e_{zmp} = ZMP_{ref} - ZMP_{real}`$
    3.  **오차 보상:** 이 오차($`e_{zmp}`$)를 0으로 만들기 위해, CoM의 목표 궤적을 수정하거나, 상체의 자세를 조절(예: 팔을 휘두르거나 허리를 숙이는 등)하는 추가적인 제어 입력을 생성합니다. 이는 주로 PID 제어기를 통해 구현됩니다.

---

**3. ZMP를 넘어서: 발 위치 조절 (Footstep Adjustment)**

ZMP 제어는 주로 상체 움직임을 통해 균형을 잡는 방식입니다. 하지만 외부의 힘이 너무 커서 상체 제어만으로는 ZMP를 지지 다각형 내에 유지할 수 없는 상황이 발생할 수 있습니다. (예: 사람이 세게 밀었을 때)
...
- **캡처 포인트 (Capture Point) 기반 제어:**
  ...
  - **활용:** 로봇은 계산된 캡처 포인트 위치로 다음 발을 빠르게 내딛어 새로운 지지 다각형을 형성하고, 넘어지지 않고 균형을 회복합니다. 이는 인간이 갑자기 밀렸을 때 본능적으로 한 발을 내딛는 행동과 매우 유사합니다.
